 ...
 * the first and second parts of this programming assignment, be sure to implement Partition exactly as described in the
 * video lectures (including exchanging the pivot element with the first element just before the main Partition
 * subroutine).
 * 
 ...
 * 
 */
public class ProblemC extends ProblemA {
    ...
    public void pivotIndex(int[] input, int left, int right) {
        ...
        int med = input[medIndex];

        if ((beg < med && med < end) || (end < med && med < beg)) {
            swap(input, left, medIndex);
        } else if ((beg < end && end < med) || (med < end && end < beg)) {
